...
void lis3dh_init_i2c(i2c_handle_t* handle) {
  i2c = handle;
}
...
u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data) {
  ...
  //i.e. SPI_Mems_Write_Reg(WriteAddr, Data);
	i2c_buffer[0] = WriteAddr;
	i2c_buffer[1] = Data;
	return (i2c_write(i2c, LIS3DH_MEMS_I2C_ADDRESS, i2c_buffer, 2) == i2cTransferDone);
}
